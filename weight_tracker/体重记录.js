...
(function () {
     ...
     * 请求确认记录体重
     */
    function requestRecordConfirmation(weight, date) {
        try {
            ...
            if (isNaN(weightValue) || weightValue <= 0 || weightValue > 500) {
                sendMessage("❌ 体重数值无效,请输入0-500之间的数字");
                return;
            }
            ...
            if (date && !isValidDate(date)) {
                sendMessage("❌ 日期格式无效,请使用 YYYY-MM-DD 格式 (如: 2026-01-01)");
                return;
            }
            ...
            // 发送确认提示
            sendMessage(`📝 准备记录 ${userName} 在 ${recordDate} 的体重: ${weightValue}kg\n\n确认记录请回复 Y, 取消请回复 Q 或 N\n(30秒内有效)`);
     ...
     * 执行记录体重
     */
    function executeRecordWeight(pendingAction) {
        try {
            const userID = getUserID();
            ...
            sendMessage(`❌ 记录体重时出错: ${error.message}`);
        }
    }
     ...
     * 显示体重记录(时间轴视图)
     */
    function showWeightRecords(days) {
        try {
            const userID = getUserID();
            const STORAGE_KEY = `user_${userID}`;
            ...
            sendMessage(`❌ 查询记录时出错: ${error.message}`);
        }
    }
    ...
    function showStatistics(days) {
        try {
            ...
            sendMessage(`❌ 查询统计时出错: ${error.message}`);
        }
    }
     ...
     * 请求确认设置目标体重
     */
    function requestSetTargetConfirmation(target) {
        try {
            const userID = getUserID();
            const PENDING_KEY = `user_${userID}`;
            ...
            if (isNaN(targetValue) || targetValue <= 0 || targetValue > 500) {
                sendMessage("❌ 目标体重数值无效,请输入0-500之间的数字");
                return;
            }
            ...
            sendMessage(`🎯 准备设置目标体重为: ${targetValue}kg\n\n确认设置请回复 Y, 取消请回复 Q 或 N\n(30秒内有效)`);

        } catch (error) {
            console.error("请求设置目标确认时出错:", error);
            sendMessage(`❌ 请求设置目标确认时出错: ${error.message}`);
        }
    }
     ...
     * 执行设置目标体重
     */
    function executeSetTargetWeight(pendingAction) {
        try {
            const userID = getUserID();
            const STORAGE_KEY = `user_${userID}`;
            ...
            sendMessage(`❌ 设置目标时出错: ${error.message}`);
        }
    }
    ...
    function showTargetProgress() {
        try {
            ...
            if (!data.target) {
                sendMessage("❌ 尚未设置目标体重\\n\\n💡 发送「设置目标体重 60」来设定目标");
                return;
            }

            if (data.records.length === 0) {
                sendMessage(`🎯 目标体重: ${data.target}kg\\n\\n📋 暂无体重记录,无法计算进度`);
                return;
            }
            ...
            sendMessage(`❌ 查询目标进度时出错: ${error.message}`);
        }
    }
     ...
     * 请求确认修改记录
     */
    function requestModifyConfirmation(indexStr, newWeight) {
        try {
            const userID = getUserID();
            const STORAGE_KEY = `user_${userID}`;
            const PENDING_KEY = `user_${userID}`;
     ...
     * 执行修改记录
     */
    function executeModifyRecord(pendingAction) {
        try {
            const userID = getUserID();
            const STORAGE_KEY = `user_${userID}`;
     ...
     * 执行清空记录
     */
    function executeClearAllRecords(pendingAction) {
        try {
            const userID = getUserID();
            const STORAGE_KEY = `user_${userID}`;
        ...
        try {
            ...
            if (content.indexOf("体重统计") !== -1) {
                ...
                if (match && match[1]) {
                    ...
                    requestSetTargetConfirmation(match[1]);
                } else {
                    sendMessage("❓ 请输入目标体重数值 (如: 设置目标体重 60)");
                }
                ...
                if (match && match[1] && match[2]) {
                    ...
                    requestModifyConfirmation(match[1], match[2]);
                } else {
                    sendMessage("❓ 指令格式错误\n正确格式: 修改体重记录 [编号] [新数值]\n示例: 修改体重记录 1 65.5");
                }
                ...
                if (recordMatch && recordMatch[1] && recordMatch[2]) {
                    ...
                    requestRecordConfirmation(recordMatch[2], recordMatch[1]);
                } else if (content.trim() === "体重记录") {
                    console.log("[体重记录插件] 执行: 查看记录");
                    showWeightRecords(7); // 默认显示最近7条
                }
                ...
                if (!isConfirmCommand(content) && !isQuitCommand(content)) {
                    ...
                    if (numMatch && numMatch[1]) {
                        ...
                        requestRecordConfirmation(numMatch[1]);
                    }
                }
            }
