...
class StomachachePlugin:
    ...
    def show_help(self):
        """显示帮助信息"""
        help_text = f"📖 肚子疼记录插件 {VERSION}\n\n"
        help_text += "🔹 功能说明：\n"
        help_text += "本插件帮助您记录和追踪肚子疼事件\n\n"
        help_text += "🔹 命令列表：\n"
        help_text += "• 肚子疼 - 记录一次肚子疼事件\n"
        help_text += "• 肚子疼记录 - 查看所有历史记录\n"
        help_text += "• 肚子疼删除 - 删除指定的历史记录\n"
        help_text += "• 肚子疼帮助 - 显示此帮助信息\n\n"
        help_text += "🔹 确认机制：\n"
        help_text += "所有操作都需要确认：\n"
        help_text += "  y - 确认执行\n"
        help_text += "  n - 取消操作\n"
        help_text += "  q - 退出流程\n\n"
        help_text += "💡 提示：记录会包含时间信息，方便您追踪症状"
    ...
    def view_records(self):
        ...
        # 构建消息
        message = f"📊 肚子疼记录 (共{total_count}条)\n"
        message += "━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    ...
    def run(self):
        ...
        try:
            # 路由到对应功能
            if self.message == "肚子疼帮助":
                self.show_help()
            elif self.message == "肚子疼记录":
                self.view_records()
            elif self.message == "肚子疼删除":
                self.delete_record()
            elif self.message == "肚子疼":
                self.record_stomachache()
            else:
                # 未匹配到命令，显示帮助
